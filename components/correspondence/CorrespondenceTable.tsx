...
type SortKey = "sentOn" | "title" | "sentBy" | "notes"

const COLUMNS: { key: SortKey; label: string }[] = [
  { key: "sentOn", label: "Date" },
...
export default function CorrespondenceTable({ docs }: { docs: CorrespondenceDoc[] }) {
  ...
  function clickHeader(key: SortKey) {
    if (key === sortKey) {
      ...
      setSortKey(key)
      setAsc(key !== "sentOn") // text columns start A→Z, date starts newest first
    }
  }
  ...
  if (docs.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <p className="text-sm text-gray-500">No letters on file yet. Correspondence will appear here as it is sent and received.</p>
      </div>
    )
  }
          ...
          return (
            ...
            >
              ...
              <dl className="mt-3 space-y-2 text-sm">
                ...
                <div className="flex gap-3">
                  <dt className={CARD_LABEL}>Sent by</dt>
                  <dd>
                    {d.sentBy ? (
                      <span
                        className="text-xs font-medium px-2 py-0.5 rounded-full border border-black/5"
                        style={{ background: sentByColor(d.sentBy).bg, color: sentByColor(d.sentBy).text }}
                      >
