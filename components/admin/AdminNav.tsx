...
export default function AdminNav({ initials }: { initials: string }) {
  ...
  return (
    <nav
      ...
      style={{ width: 80, background: "#F5EEE3", borderColor: "#E8DFD2" }}
      aria-label="Admin navigation"
    >
      <Link href="/admin" className="mb-2" aria-label="Home">
        <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-[11px] font-bold tracking-wider" style={{ background: "#1B2D45" }}>
          EFL
        </div>
      </Link>
        ...
        {ITEMS.map((it) => {
          ...
          return (
            <Link
              ...
              style={{ background: active ? "#1B2D45" : "transparent", color: active ? "#fff" : "#334155" }}
            >
      ...
      <div className="mt-auto flex flex-col items-center gap-2">
        <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-semibold" style={{ background: "#F0E7DA", color: "#334155" }} title="Signed in">
          {initials}
        </div>
